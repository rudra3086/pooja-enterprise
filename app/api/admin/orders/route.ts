...
import { getSessionByToken, getOrders, deleteOrders } from "@/lib/db"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { orders, total } = await getOrders({
      ...
      excludeAdminHidden: view !== "removed",
      onlyAdminHidden: view === "removed",
      limit: pageSize,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const deletedCount = await deleteOrders({
    ...
    return NextResponse.json<ApiResponse<{ deletedCount: number }>>({
      success: true,
      data: { deletedCount },
      message: `Removed ${deletedCount} order(s) from admin UI successfully`,
    })
  } catch (error) {
